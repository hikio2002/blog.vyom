...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ tablets, total, page, totalPages: Math.ceil(total / limit) });
  } catch (e: any) {
    console.error('GET /api/tablets error:', e);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const exists = await Tablet.findOne({ slug });
    if (exists) return NextResponse.json({ error: 'A tablet with this slug already exists' }, { status: 409 });

    const tablet = await Tablet.create({ ...body, slug });

    revalidatePath('/tablets');
    revalidatePath(`/tablets/${slug}`);

    return NextResponse.json(tablet, { status: 201 });
  } catch (e: any) {
    console.error('POST /api/tablets error:', e);
    return NextResponse.json({ error: e.message || 'Internal server error' }, { status: 500 });
  }
}
