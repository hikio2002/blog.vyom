...
import { Plus, Pencil, Trash2, ExternalLink, Laptop as LaptopIcon, Star } from 'lucide-react';
...
export default function AdminLaptopsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/laptops?${p}`, { headers: headers() });
      const data = await safeJson<{ laptops: Laptop[]; total: number; totalPages: number }>(res);
      if (res.ok && data) {
  ...
  useEffect(() => {
    fetch('/api/laptop-categories', { headers: headers() })
      .then(r => safeJson<LaptopCategory[]>(r))
      .then(d => { if (Array.isArray(d)) setCategories(d); })
      .catch(() => {});
  }, [headers]);

  const del = async (laptop: Laptop) => {
    if (!confirm(`Delete "${laptop.brand} ${laptop.name}"?`)) return;
    try {
      const res = await fetch(`/api/laptops/${laptop._id}`, { method: 'DELETE', headers: headers() });
  ...
  const toggle = async (laptop: Laptop) => {
    try {
      await fetch(`/api/laptops/${laptop._id}`, {
        method: 'PUT',
        headers: { ...headers(), 'Content-Type': 'application/json' },
        body: JSON.stringify({ isActive: !laptop.isActive }),
      });
      load();
    } catch {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100" style={{ fontFamily: 'var(--font-syne)' }}>Laptops</h1>
          <p className="text-sm text-gray-400 mt-0.5">{total} total</p>
        </div>
        <Link href="/admin/laptops/new" className="btn-primary gap-2"><Plus size={16} />Add Laptop</Link>
      </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 text-center text-gray-400">
            <LaptopIcon size={32} className="mx-auto mb-3 opacity-30" />
            <p>No laptops yet. <Link href="/admin/laptops/new" className="text-indigo-600 dark:text-indigo-400 hover:underline">Add one →</Link></p>
          </div>
        ) : (
                  ...
                  <tr key={p._id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        {p.isActive && (
                          <a href={`/laptops/${p.slug}`} target="_blank" rel="noopener noreferrer" title="View live"
                            className="p-1.5 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors rounded">
                            <ExternalLink size={14} />
                          </a>
                        )}
                        <Link href={`/admin/laptops/${p._id}`} title="Edit" className="p-1.5 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors rounded">
                          <Pencil size={14} />
                        </Link>
