...
import { Plus, Pencil, Trash2, ExternalLink, PenTool, Star } from 'lucide-react';
...
export default function AdminTabletsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/tablets?${p}`, { headers: headers() });
      const data = await safeJson<{ tablets: Tablet[]; total: number; totalPages: number }>(res);
      if (res.ok && data) {
  ...
  useEffect(() => {
    fetch('/api/tablet-categories', { headers: headers() })
      .then(r => safeJson<TabletCategory[]>(r))
      .then(d => { if (Array.isArray(d)) setCategories(d); })
      .catch(() => {});
  }, [headers]);

  const del = async (tablet: Tablet) => {
    if (!confirm(`Delete "${tablet.brand} ${tablet.name}"?`)) return;
    try {
      const res = await fetch(`/api/tablets/${tablet._id}`, { method: 'DELETE', headers: headers() });
  ...
  const toggle = async (tablet: Tablet) => {
    try {
      await fetch(`/api/tablets/${tablet._id}`, {
        method: 'PUT',
        headers: { ...headers(), 'Content-Type': 'application/json' },
        body: JSON.stringify({ isActive: !tablet.isActive }),
      });
      load();
    } catch {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100" style={{ fontFamily: 'var(--font-syne)' }}>Drawing Tablets</h1>
          <p className="text-sm text-gray-400 mt-0.5">{total} total</p>
        </div>
        <Link href="/admin/tablets/new" className="btn-primary gap-2"><Plus size={16} />Add Tablet</Link>
      </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 text-center text-gray-400">
            ...
            <p>No drawing tablets yet. <Link href="/admin/tablets/new" className="text-indigo-600 dark:text-indigo-400 hover:underline">Add one →</Link></p>
          </div>
        ) : (
                  ...
                  <tr key={p._id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        {p.isActive && (
                          <a href={`/tablets/${p.slug}`} target="_blank" rel="noopener noreferrer" title="View live"
                            className="p-1.5 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors rounded">
                            <ExternalLink size={14} />
                          </a>
                        )}
                        <Link href={`/admin/tablets/${p._id}`} title="Edit" className="p-1.5 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors rounded">
                          <Pencil size={14} />
                        </Link>
