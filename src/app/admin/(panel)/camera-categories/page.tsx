...
const empty = { name: '', slug: '', description: '', icon: 'Camera', order: '0', isActive: true };

const ICON_OPTIONS = ['Camera', 'Wallet', 'Crown', 'Camera', 'Gamepad2', 'Star', 'Zap', 'Award'];

export default function AdminPhoneCategoriesPage() {
  const [cats, setCats] = useState<CameraCategory[]>([]);
  const [loading, setLoading] = useState(true);
  const [modal, setModal] = useState<'new' | CameraCategory | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/camera-categories', { headers: headers() });
      const data = await safeJson<CameraCategory[]>(res);
  ...
  const openEdit = (c: CameraCategory) => {
    setForm({
      name: c.name, slug: c.slug, description: c.description || '',
      icon: c.icon || 'Camera', order: String(c.order), isActive: c.isActive,
    });
    setModal(c);
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      const url = isEdit ? `/api/camera-categories/${(modal as CameraCategory)._id}` : '/api/camera-categories';
  ...
  const toggle = async (c: CameraCategory) => {
    try {
      const res = await fetch(`/api/camera-categories/${c._id}`, {
  ...
  const del = async (c: CameraCategory) => {
    if (!confirm(`Delete "${c.name}"? Phones in this category must be moved/deleted first.`)) return;
    try {
      const res = await fetch(`/api/camera-categories/${c._id}`, { method: 'DELETE', headers: headers() });
      const data = await safeJson<any>(res);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">e.g. DSLR, Mirrorless, Point & Shoot, Action Cams</p>
        </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.name} onChange={e => { set('name', e.target.value); if (modal === 'new') set('slug', makeSlug(e.target.value)); }} className="input" placeholder="e.g. Mirrorless" />
              </div>
              <div>
