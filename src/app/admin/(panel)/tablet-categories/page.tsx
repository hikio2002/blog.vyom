...
import type { TabletCategory } from '@/types';
import Cookies from 'js-cookie';

const empty = { name: '', slug: '', description: '', icon: 'PenTool', order: '0', isActive: true };

const ICON_OPTIONS = ['PenTool', 'Wallet', 'Crown', 'Camera', 'Gamepad2', 'Star', 'Zap', 'Award'];

export default function AdminPhoneCategoriesPage() {
  const [cats, setCats] = useState<TabletCategory[]>([]);
  const [loading, setLoading] = useState(true);
  const [modal, setModal] = useState<'new' | TabletCategory | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/tablet-categories', { headers: headers() });
      const data = await safeJson<TabletCategory[]>(res);
  ...
  const openEdit = (c: TabletCategory) => {
    setForm({
      name: c.name, slug: c.slug, description: c.description || '',
      icon: c.icon || 'PenTool', order: String(c.order), isActive: c.isActive,
    });
    setModal(c);
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      const url = isEdit ? `/api/tablet-categories/${(modal as TabletCategory)._id}` : '/api/tablet-categories';
  ...
  const toggle = async (c: TabletCategory) => {
    try {
      const res = await fetch(`/api/tablet-categories/${c._id}`, {
  ...
  const del = async (c: TabletCategory) => {
    if (!confirm(`Delete "${c.name}"? Phones in this category must be moved/deleted first.`)) return;
    try {
      const res = await fetch(`/api/tablet-categories/${c._id}`, { method: 'DELETE', headers: headers() });
      const data = await safeJson<any>(res);
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100" style={{ fontFamily: 'var(--font-syne)' }}>
            Drawing Tablet Categories
          </h1>
          <p className="text-sm text-gray-400 mt-0.5">e.g. Entry-level, Professional, Display Tablets</p>
        </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.name} onChange={e => { set('name', e.target.value); if (modal === 'new') set('slug', makeSlug(e.target.value)); }} className="input" placeholder="e.g. Professional" />
              </div>
              <div>
