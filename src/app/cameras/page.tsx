...
import { getActiveCameraCategories, getFeaturedCameras, getSiteSettings } from '@/lib/server-api';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: `Camera Finder | ${settings.siteName}`,
    description: 'Browse cameras by category — with full specs, pricing, and pros & cons.',
  };
}

export default async function CamerasPage() {
  const [categories, featured] = await Promise.all([
    getActiveCameraCategories(),
    getFeaturedCameras(8),
  ]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        ...
        <div className="mb-8">
          <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-3" style={{ fontFamily: 'var(--font-syne)' }}>
            <CameraIcon className="text-indigo-500" size={32} />
            Camera Finder
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-2">
            Find the right camera by category — with full specs, pricing, and pros &amp; cons.
          </p>
        </div>
          ...
          <div className="flex flex-wrap gap-3 mb-10">
            {(categories as any[]).map(cat => {
              ...
              return (
                <Link
                  key={String(cat._id)}
                  href={`/cameras/category/${cat.slug}`}
        ...
        {featured.length > 0 ? (
          <section>
            <h2 className="section-title mb-5">Featured Cameras</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
              {(featured as any[]).map(p => <CameraCard key={String(p._id)} camera={p as CameraType} />)}
            </div>
          </section>
        ) : categories.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            <CameraIcon size={32} className="mx-auto mb-3 opacity-30" />
            <p className="text-lg">Camera categories coming soon!</p>
          </div>
        ) : null}
      </div>
    </PublicLayout>
