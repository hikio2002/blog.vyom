...
import { Bold, Italic, Strikethrough, Code, Heading1, Heading2, Heading3, List, ListOrdered, Quote, Minus, Link as LinkIcon, Image as ImageIcon, Table as TableIcon, Youtube as YoutubeIcon, Undo, Redo, Code2, RemoveFormatting } from 'lucide-react';
...
export default function RichEditor({ value, onChange }: Props) {
  const editor = useEditor({
    ...
    editorProps: { attributes: { class: 'prose prose-base dark:prose-dark max-w-none focus:outline-none px-4 py-4 min-h-96' } },
  });
  ...
  const btn = (action: () => void, label: string, active = false) => (
    <button type="button" title={label} onClick={action}
      className={`p-1.5 rounded text-xs transition-colors ${active ? 'bg-brand-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
      {label}
    </button>
  );
  const iconBtn = (action: () => void, Icon: any, label: string, active = false) => (
    <button type="button" title={label} onClick={action}
      className={`p-1.5 rounded transition-colors ${active ? 'bg-brand-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
      <Icon size={14} />
    </button>
  );
  ...
  return (
    <div className="border border-gray-300 dark:border-gray-600 rounded-xl overflow-hidden bg-white dark:bg-gray-900">
      ...
      <div className="flex flex-wrap items-center gap-0.5 p-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
        <div className="flex items-center gap-0.5 pr-2 border-r border-gray-200 dark:border-gray-700 mr-1">
