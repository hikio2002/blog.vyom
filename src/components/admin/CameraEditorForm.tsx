...
const SPEC_FIELDS: { key: keyof Camera['specs']; label: string; placeholder: string }[] = [
  { key: 'sensorType',      label: 'Sensor Type',       placeholder: 'Full-frame CMOS' },
  { key: 'resolution',      label: 'Resolution',         placeholder: '24.2 MP' },
  { key: 'lensMount',       label: 'Lens Mount',         placeholder: 'Sony E-mount' },
  { key: 'iso',             label: 'ISO Range',          placeholder: '100 - 51200' },
  { key: 'videoResolution', label: 'Video Resolution',   placeholder: '4K 60fps' },
  { key: 'autofocus',       label: 'Autofocus',          placeholder: '693-point Phase Detection' },
  { key: 'stabilization',   label: 'Stabilization',      placeholder: '5-axis In-Body (IBIS)' },
  { key: 'battery',         label: 'Battery Life',       placeholder: '~600 shots per charge' },
  { key: 'weight',          label: 'Weight',             placeholder: '650g (body only)' },
  { key: 'dimensions',      label: 'Dimensions',         placeholder: '12.9 x 9.7 x 8.0 cm' },
];

export default function CameraEditorForm({ camera }: Props) {
  ...
  const [category, setCategory] = useState((camera?.category as any)?._id || '');
  const [price, setPrice] = useState(camera?.price ? String(camera.price) : '');
  const [currency, setCurrency] = useState(camera?.currency || '₹');
  const [images, setImages] = useState<string[]>(camera?.images?.length ? camera.images : ['']);
  const [description, setDescription] = useState(camera?.description || '');
  const [specs, setSpecs] = useState<Camera['specs']>(camera?.specs || {});
  const [pros, setPros] = useState<string[]>(camera?.pros?.length ? camera.pros : ['']);
  const [cons, setCons] = useState<string[]>(camera?.cons?.length ? camera.cons : ['']);
  const [rating, setRating] = useState(camera?.rating ? String(camera.rating) : '');
  const [buyLink, setBuyLink] = useState(camera?.buyLink || '');
  const [isActive, setIsActive] = useState(camera?.isActive ?? true);
  const [isFeatured, setIsFeatured] = useState(camera?.isFeatured ?? false);
  ...
  useEffect(() => {
    ...
    fetch('/api/camera-categories', { headers: { Authorization: `Bearer ${token}` } })
  ...
  const setSpecField = (key: keyof Camera['specs'], val: string) =>
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!isEdit) router.push(`/admin/cameras/${data._id}`);
    } catch (e: any) {
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
        <div>
          ...
          {camera?.slug && <p className="text-xs text-gray-400 mt-0.5">/cameras/{camera.slug}</p>}
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {camera?.slug && (
            <a href={`/cameras/${camera.slug}`} target="_blank" rel="noopener noreferrer" className="btn-outline gap-1.5 text-xs py-2">
              Preview
            </a>
          )}
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="card p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input value={brand} onChange={e => setBrand(e.target.value)} placeholder="Sony, Canon, Nikon, Fujifilm…" className="input" />
            </div>
            <div>
              <label className="label">Model Name *</label>
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Alpha a7 IV, EOS R6, Z6 III" className="input" />
            </div>
            ...
            <div className="flex gap-2">
              ...
              <div className="flex-1">
                ...
                <input type="number" value={price} onChange={e => setPrice(e.target.value)} placeholder="129999" className="input" />
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2">
              {images.map((img, i) => (
                <div key={i} className="flex gap-2 items-center">
                  <input value={img} onChange={e => imgOps.update(i, e.target.value)} placeholder="https://example.com/camera.jpg" className="input text-xs flex-1" />
                  {img && (
